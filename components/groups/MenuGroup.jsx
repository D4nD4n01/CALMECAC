...
const MenuGroup = ({ route, navigation }) => {
    const { nombre, idGrupo } = route.params;

    useEffect(() => {
        console.log("parametros: ",route)
        const estudiantesGrupo = alumnos.filter(a => a.idGrupo === idGrupo);
    ...
    const handleVerAsistencia = () => {
      ...
      return (
        <View style={{ flex: 1, justifyContent: "center", padding: 20 }}>
          <Text style={{ fontSize: 20, marginBottom: 20 }}>Grupo: {nombre}</Text>
    
          <TouchableOpacity
            onPress={handleVerAsistencia}
            style={{ backgroundColor: "#007bff", padding: 12, borderRadius: 8, marginBottom: 10 }}
          >
            <Text style={{ color: "white", textAlign: "center" }}>Ver Asistencia</Text>
          </TouchableOpacity>
          ...
          >
            <Text style={{ color: "white", textAlign: "center" }}>Buscar Alumno</Text>
          </TouchableOpacity>
        </View>
      );
    };
